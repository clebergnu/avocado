...
def register_core_options():
    ...
    help_msg = (
        f"List of comma separated builtin logs, or logging streams "
    ...
    help_msg = (
        "Whether to force colored output to non-tty outputs "
    ...
    help_msg = (
        "Use a custom exit code list to consider a test as skipped. "
        "This is only used by exec-test runners. Default is []."
    )
    stgs.register_option(
        section="runner.exectest.exitcodes",
        key="skip",
        default=[],
        key_type=list,
        help_msg=help_msg,
    ...
    stgs.register_option(
        section="runner", key="identifier_format", default="{uri}", help_msg=help_msg
    )
