...
class TaskStatusService:
    ...
    def open_connection(self):
        ...
        port = int(port)
        _, results = yield from asyncio.open_connection(host, port)
        return results

    @asyncio.coroutine
    def post(self, status):
        if self.connection is None:
            self.connection = yield from self.open_connection()
    ...
    def run(self):
        ...
        for status in runner.run():
            ...
            for status_service in self.status_services:
                yield from status_service.post(status)
            yield status
...
def subcommand_task_run(args, echo=print):
    ...
    loop = asyncio.get_event_loop()
    loop.run_until_complete(task_run(task, echo))
