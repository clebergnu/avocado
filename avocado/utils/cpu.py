...
def get_idle_state():
    ...
    for cpu in cpus_list:
        ...
        for state_no in range(states):
            ...
            try:
                with open(state_file, "rb") as state_fo:  # pylint: disable=W1514
                    state = state_fo.read()
                cpu_idlestate[cpu][state_no] = bool(int(state))
    ...
    if not setstate:
        ...
        for cpu in cpus_list:
            for state_no in states:
                ...
                try:
                    with open(state_file, "wb") as state_fo:  # pylint: disable=W1514
                        state_fo.write(disable)
                except IOError as err:
                    LOG.warning(
