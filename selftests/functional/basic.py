...
class RunnerOperationTest(TestCaseTmpDir):
    def test_show_version(self):
        ...
        self.assertTrue(
            re.match(r"^Avocado \d+\.\d+$", result.stdout_text),
            (
    ...
    def test_unsupported_status(self):
        with script.TemporaryScript(
            ...
            self.assertEqual(
                ...
                "ERROR",
                (f"{results['tests'][0]['status']} != " f"{'ERROR'}\n{res}"),
            )
            self.assertIn(
    ...
    def test_runner_doublefail(self):
        ...
        self.assertIn(
            b"TestError: Failing during tearDown. Yay!",
            result.stdout,
            "Cleanup exception not printed to log output",
        )
        self.assertIn(
            b"TestFail: This test is supposed to fail",
            result.stdout,
            (f"Test did not fail with action exception:" f"\n{result.stdout}"),
        )
    ...
    def test_exception_not_in_path(self):
        ...
        mylib = script.Script(
            os.path.join(self.tmpdir.name, "shared_lib", "mylib.py"),
        ...
        mytest = script.Script(
            os.path.join(self.tmpdir.name, "mytest.py"),
        ...
        self.assertIn(
            b"'fail_reason': 'This should not crash on " b"unpickling in runner'",
            result.stdout,
        )
    ...
    def test_show_user_stream(self):
        cmd_line = (
            f"{AVOCADO} --show=app,avocado.test.progress run "
    ...
    def test_runner_test_parameters(self):
        ...
        self.assertIn(
            b"PARAMS (key=sleep_length, path=*, default=1) => '0.01'", test_log
        )
    ...
    def test_other_loggers(self):
        with script.TemporaryScript(
            "mytest.py",
            TEST_OTHER_LOGGERS_CONTENT,
            "avocado_functional_test_other_loggers",
        ) as mytest:
...
class RunnerHumanOutputTest(TestCaseTmpDir):
    ...
    def test_output_cancel(self):
        ...
        self.assertIn(
            b"PASS 0 | ERROR 0 | FAIL 0 | SKIP 0 | WARN 0 | " b"INTERRUPT 0 | CANCEL 1",
            result.stdout,
        )
...
class PluginsTest(TestCaseTmpDir):
    ...
    def test_list_plugin(self):
        ...
        self.assertNotIn(b"No tests were found on current tests dir", result.stdout)
    ...
    def test_disable_plugin(self):
        ...
        config = script.TemporaryScript("disable_sysinfo_cmd.conf", config_content)
        with config:
...
class PluginsXunitTest(TestCaseTmpDir):
    ...
    def run_and_check(self, testname, e_rc, e_ntests, e_nerrors, e_nfailures, e_nskip):
        ...
        self.assertEqual(
            n_skip, e_nskip, f"Unexpected number of test skips, XML:\n" f"{xml_output}"
        )
    ...
    def test_xunit_plugin_skiponsetuptest(self):
        self.run_and_check(
            "examples/tests/cancelonsetup.py", exit_codes.AVOCADO_ALL_OK, 1, 0, 0, 1
        )
...
class PluginsJSONTest(TestCaseTmpDir):
    def run_and_check(
        self, testname, e_rc, e_ntests, e_nerrors, e_nfailures, e_nskip, e_ncancel=0
    ):
        cmd_line = (
            f"{AVOCADO} run --job-results-dir {self.tmpdir.name} "
