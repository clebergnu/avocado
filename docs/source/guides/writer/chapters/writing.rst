...
On those circunstances, it's possible to set a "timeout factor".  This
is pretty much a multiplier to the given timeout (either set in the
class, or given as a parameter).  Example::
...
Notice how, under normal circunstances, the test would have timed out,
